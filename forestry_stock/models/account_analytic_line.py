...
class AccountAnalyticLine(models.Model):
    ...
    def _action_apply_inventory(
        self, product_id, product_stock_uom_id, product_qty, operation="remove"
    ):
        """Update on hand qty."""
        ...
        if self.task_id and product_id.type == "product":
            ...
            if operation == "add":
                ...
                if quant.quantity < 0.0:
                    raise UserError(
                        _(
                            "No empty or positive inventory has been found for product %s."
                        )
                        % product_id.display_name
                    )
                quant.inventory_quantity = quant.quantity + line_quantity
