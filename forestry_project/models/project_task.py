...
class ProjectTask(models.Model):
    ...
    location_link = fields.Char(
        "Source Location Link", related="location_id.location_link", readonly=False
    )

    product_dest_id = fields.Many2one(
        "product.product", "Target Product", check_company=True
    )
    location_dest_id = fields.Many2one(
        "res.partner", "Destination Location", check_company=True
    )
    location_dest_link = fields.Char(
        "Destination Location Link",
        related="location_dest_id.location_link",
        readonly=False,
    )
    ...
    def create(self, vals_list):
        ...
        date_begin = (
            date_begin.replace(hour=8).astimezone(pytz.utc).replace(tzinfo=None)
        )
